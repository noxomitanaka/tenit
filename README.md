...
今日の予約・アクティブ会員・振替残数・未払い件数を一覧表示。

![Dashboard](./docs/screenshots/01-dashboard.png)

### 予約一覧 (Reservations)
コート・レッスン単位の予約状況を一覧で管理。

![Reservations](./docs/screenshots/02-reservations-calendar.png)

### スケジュール (Schedule & Substitutes)
月次カレンダーでレッスンスロットを管理。振替対象スロットへの誘導もここから。

![Schedule](./docs/screenshots/03-substitute-schedule.png)

### QR出席管理 (QR Attendance)
バーコードリーダーまたは QR スキャンでワンタッチ打刻。手動追加にも対応。

![QR Attendance](./docs/screenshots/04-qr-attendance.png)

### モバイル会員ポータル (Member Portal — Mobile)
会員は自分の予約・振替クレジット・月謝をモバイルから確認。

![Mobile Portal](./docs/screenshots/05-mobile-portal.png)

> 撮影スクリプト: `docs/capture-screenshots.spec.ts` (Playwright)。`npm run dev` 起動後、`npx playwright test --config docs/playwright.screenshots.config.ts` で再生成可能。
